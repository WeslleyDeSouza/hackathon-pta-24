...
export class UserStoryFacade {
  constructor(
    @InjectRepository(UserStoryEntity) protected userStoryEntity: Repository<UserStoryEntity>
  ) {}

  listByProjectId(tenantId: number, projectId: number, options?: { withEstimation: boolean }) {
    return this.userStoryEntity.find({
      where: {
        tenantId,
        projectId,
      },
      relations: options?.withEstimation ? ["estimations", "estimations.user"] : [],
    });
  }
