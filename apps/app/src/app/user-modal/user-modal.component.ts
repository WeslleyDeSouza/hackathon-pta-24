...
import { BadgeUserAchievementDtoResponse } from '@hackathon-pta/api/model/badge';
...
export class UserModalComponentComponent implements OnInit {
  ...
  ngOnInit() {
    this.badgeListAchieved$ = this.badgeService.badgeListByUserId({id: this.userId}).pipe(
      map(x => x.filter(b => b.achieved))
    );
    this.badgeListNotAchieved$ = this.badgeService.badgeListByUserId({id: this.userId}).pipe(
      map(x => x.filter(b => !b.achieved))
    );
  }
}
